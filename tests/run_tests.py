...
t0 = time.time()
if len(sys.argv) > 1: # If there are any command line parameters given
    if sys.argv[1] == '-v':
        tests.run(constants, verbose=True)
    else:
        print('Usage: python run_test.py [-v], where -v means verbose and will result in more detailed output for each test.')
else:
    tests.run(constants)
t1 = time.time()
